                ...
                <section class="section">
                    <div class="card">
                        @if(Auth::user()->can('ACP-role-create'))
                            <a href="{{ route('acp.role.create') }}" class="btn btn-primary">{{ __('Create new role') }}</a>
                        @else
                            <button href="" class="btn btn-primary" disabled>{{ __('Create new role') }}</button>
                        @endif
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                            @if(Auth::user()->can('ACP-role-edit'))
                                                <a href="{{ route('acp.role.edit', $item->id) }}" type="button" class="btn btn-outline-info">Edit</a>
                                            @else
                                            ...
                                            @else
                                                <button href="#" type="button" class="btn btn-outline-danger" disabled>Delete</button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                                <div class="modal fade text-left" id="danger{{ $item->id }}" tabindex="-1"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <form method="post" action="{{ route('acp.role.delete', $item->id) }}">
                                                @method('patch')
                                                @csrf
