...
@section('content')
    <div class="page-content">
        <section class="section">
            <div class="card">
                @if(Auth::user()->can('ACP-parking-create'))
                <a href="{{ route('acp.parking.create') }}" class="btn btn-primary">{{ __('Create new parking') }}</a>
                @else
                    <button class="btn btn-primary" disabled>{{ __('Create new parking') }}</button>
                @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                        @if(Auth::user()->can('ACP-parking-edit'))
                                        <a href="{{ route('acp.parking.edit', $item->id) }}" type="button" class="btn btn-outline-info">Edit</a>
                                        @else
                                            <button class="btn btn-outline-info" disabled>Edit</button>
                                        @endif
                                        @if(Auth::user()->can('ACP-parking-edit') && Auth::user()->can('ACP-parking-delete'))
                            ...
                            <div class="modal fade text-left" id="danger{{ $item->id }}" tabindex="-1"
                                ...
                                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form method="post" action="{{ route('acp.parking.delete', $item->id) }}">
                                            @method('patch')
                                            @csrf
                                            <div class="modal-body">
                                                {{ __('Parking') }} <b>{{$item->name}}</b> {{ __('will be deleted and cannot be restored.') }}

                                                <hr>
                                                <div class="form-group row">
                                                    <label for="IsActive{{$item->id}}" class="col-md-11 col-form-label">Confim delete parking</label>
